...
Atomist provides an external DSL for authoring **project editors** and
**project reviewers**, codenamed **Rug**, in honor of the Dude.  Rug
ties things together.

**Project editors** are named units of functionality that can be used
to update one or more projects.  The typical Atomist workflow is to
create a project which contains static content as a starting point and
a number of editors that can be used to create new projects from it
and help users to evolve created projects.

**Project reviewers** are similar modules to project editors, but do
not update projects, but report potential problems with them.

## Basic Concepts

Rug is a simple, English-like, DSL. It does not contain a full-blown
set of control structures and this is not its purpose.
...
Editors are packaged in a fixed directory structure. Normally this is
sourced from a GitHub. All editors must be in or under the
`/.atomist/editors` directory the repo. Editors have access to
template content in the same archive, packaged under
`/.atomist/templates`.

>**Remember: All Atomist files should be under the `.atomist` directory in the root of a project.**

Editor files must have a `.rug` extension.  A `.rug` file can contain
one or more editors and reviewers.  A `.rug` file must contain either:

*   A single project operation with the same name as the project file,
    excluding the `.rug` extension. This corresponds to Java's
    enforcement of the packaging of public classes. *-or-*
*   A single project operation with the same name as the project file,
    excluding the `.rug` extension, plus any number of other project
    operations that are used only by that project operation.
...
An editor annotated with the `@generator` annotation can be used as a
**project generator**, acting on the content of the repo it is located
in, excluding the `.atomist` directly. A single repo can contain
multiple project generator editors.
...
Rug is designed to make the definition of project operations concise
and readable.
...
This means *for each file in the project if name contains ".txt"
append the given string to the end of the file.* The code between
`when` and `do` is a *predicate*, specifying which files to match.
...
modify files in ways determined by the caller. (Parameters are
typically entered by a user, prompted by the Atomist bot or a web
form.) Note that the parameter definition specifies a regular
expression that will be used to validate it before it's passed to the
editor. So the editor's implementation can assume that it's valid upon
execution.
...
We can compose predicates used with `with`. In the following example,
we `and` two tests on a file to narrow matching:
...
with File f when name contains ".txt" and { 13 < 27 }
  do append "42"
```
...
in separate files within the same archive. We could also refer to
editors outside the current archive, depending on which project
operations are loaded in the current context.
...
**Reviewers** are programs with similar structure to editors. They
also use parameters, computed values, `with` blocks with predicates
and `do` blocks. However, they cannot make changes to projects, but
...
with JavaClass c when imports "javax.ejb"
  do majorProblem "This file uses EJB"
```
...
**Postconditions** can be used to verify the result of editing. An
editor that fails to meet its postcondition will not make any changes,
but throw an error message.  Thus including postconditions makes an
editor more robust as well as self-documenting.
...
`file` here is not a function or a reserved word in Rug, but a
**type**.  Types are an important extension point of Rug, enabling it
to expose a wide range of functionality in a native-seeming way.
Types support a range of methods, which can be used in `with`
predicates or `do` statements.

A number of types are shipped out of the box. The most important are:

* Project
* File
* JavaSource, JavaClass, JavaMethod
* Docker
...
Rug ships with support for Java out of the box, defining `class` and
`method` types. For example, this editor adds an annotation to classes
whose name contain a given pattern:
...
with Class c when { c.name().contains(name_match) }
  do addAnnotation annotation_package annotation
```

## Escaping into JavaScript

Rug is intentionally limited, aiming for readability rather than
power.  However, it makes it easy
to [escape to JavaScript][javascript] at any time to perform more
complex tasks.
...
Rug can be extended by via new **types**, which expose an element of
the target project structure to convenient manipulation.

The `com.atomist.rug.spi` package provides interfaces to be extended
